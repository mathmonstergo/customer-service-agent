...
def build_openai_client(base_url: str, api_key: str, *, timeout: float | None = None) -> OpenAI:
    """构造 OpenAI-compatible client；关键约束是所有外部调用必须有超时边界。"""
...
class EmbeddingClient:
    ...
    def from_settings(cls, settings: Settings) -> "EmbeddingClient":
        client = build_openai_client(
            ...
            timeout=getattr(settings, "embedding_timeout_seconds", 30.0),
        )
        return cls(
            client,
...
class ChatClient:
    ...
    def from_settings(cls, settings: Settings) -> "ChatClient":
        client = build_openai_client(
            ...
            timeout=getattr(settings, "chat_timeout_seconds", 60.0),
        )
...
class RerankClient:
    ...
    def from_settings(cls, settings: Settings | Any) -> "RerankClient | None":
        ...
        timeout = float(getattr(settings, "rerank_timeout_seconds", 30.0) or 30.0)
        return cls(
            base_url=base_url,
            api_key=api_key,
            model=model,
