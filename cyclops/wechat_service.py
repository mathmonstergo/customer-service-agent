...
from cyclops.rag import RagService
...
def run_service(settings: Settings) -> None:
    """运行微信长连接服务；关键约束是单机只允许一个 Cyclops 实例持有轮询锁。"""
    from cyclops.cli import build_rag

    lock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    try:
        lock.bind(("127.0.0.1", 19529))
    except OSError:
        print("[WeChat] Another Cyclops service instance is running", file=sys.stderr)
        raise SystemExit(1)

    bot = WxBotClient(settings.wechat_token_file)
    if not bot.token:
        raise RuntimeError("WeChat token missing. Run `cyclops wechat-login` first.")
